...
describe('AdminPermissionsComponent', () => {
  ...
  const matrix: PermissionAssignment[] = [
    { id: 'p1', roleId: 'r1', componentId: 'c1', actionCode: 'VIEW', permissionCode: 'MATCHER.VIEW' },
  ];

  beforeEach(async () => {
    permSpy = jasmine.createSpyObj<AdminPermissionService>('AdminPermissionService', ['getActions', 'getMatrix', 'saveMatrix']);
    permSpy.getActions.and.returnValue(of(actions));
    permSpy.getMatrix.and.returnValue(of(matrix));
    permSpy.saveMatrix.and.returnValue(of(void 0));
  ...
  it('builds matrix for selected role', () => {
  ...
  it('toggle adds/removes assignment', () => {
    ...
    component.form.get('roleId')?.setValue('r1');
  ...
  it('save calls service', () => {
    component.save();
    expect(permSpy.saveMatrix).toHaveBeenCalled();
  });
});
